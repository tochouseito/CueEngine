...
{
    if (a_count == 5 && std::string_view(a_arguments[1]) == "--preset" && std::string_view(a_arguments[3]) == "-B")
    {
        std::cout << "configure:" << a_arguments[2] << '\n';
        return 0;
    }
    if (a_count == 7 && std::string_view(a_arguments[1]) == "--build" &&
        std::string_view(a_arguments[3]) == "--config" && std::string_view(a_arguments[5]) == "--target" &&
        std::string_view(a_arguments[6]) == "CueGameModule")
    {
