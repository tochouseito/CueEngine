...
{
    cue::RuntimeHost invalidCallbacks({{"CueRuntimeHost Test", {320, 240}}, {2, true, 0}, 0});
    ...
    cue::RuntimeHost invalidWindow({{"", {320, 240}}, {2, true, 0}, 0});
    ...
    cue::RuntimeHost invalidFrame({{"CueRuntimeHost Invalid Frame", {320, 240}}, {0, true, 0}, 0});
...
/// @brief Callbackの差替え、Frame進行、自動終了、停止を両Thread構成で確認する
int test_frame_callbacks(bool a_useWorkerThreads)
{
    std::atomic<std::uint64_t> updates = 0;
    std::atomic<std::uint64_t> renders = 0;
    cue::RuntimeHost host({{"CueRuntimeHost Test", {320, 240}}, {2, a_useWorkerThreads, 0}, 4});
    ...
    while (std::chrono::steady_clock::now() < deadline)
    {
        ...
        if (!host.frame_controller().step().has_value())
        {
            return 3;
        }
    }
    const auto progress = host.frame_controller().progress();
...
{
    cue::RuntimeHost host({{"CueRuntimeHost Failure", {320, 240}}, {2, true, 0}, 0});
    auto initResult = host.initialize(
    ...
    while (std::chrono::steady_clock::now() < deadline)
    {
        auto result = host.frame_controller().step();
        if (!result.has_value())
        {
...
/// @brief 実Window上のRuntimeHostが失敗後も資源を残さないことを確認する
int main()
{
